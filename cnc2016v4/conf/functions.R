FIS= function(layers){
 ...
 ############################################################
  ...
  fis_trend_gl <- fis_gl %>%
   ...
   trend_gl<-as.data.frame(fis_trend_gl)

  #final formatting for status from global model/senario
  fis_status_gl <- fis_gl %>%
  ...
  scores_FIS <-  rbind(fis_status_gl, fis_trend_gl)%>%
    mutate(goal = 'FIS',
...
LSP = function(layers){
  ...
  #select data ----
    ...
    lsp_status_allyear <- lsp_final_data %>%
      ...
      fill(cum_rel_prot) %>%
      # ## v2: Reference point is 1288864.900 minus the remaining protected areas on the list (total cumulative protection in 2016 = 1287798.706) at IUNC category VI (score = 0.1)
      mutate(ref_point_2 = 1287798.706 * 0.1 * 1,
             status_2 = round((cum_rel_prot / ref_point_2) * 100, 2)) %>%
      select(year, score = status_2)

    #proposing different reference points
    ##MANAGEMENT: all versions are at Management Plan Implemented = 1

    # ## v1: Reference point: Reaching 70% of EZZ as Cat IV, 30% of entire EEZ as Cat I (working group)
    # mutate(ref_point = (((1287798.706 * .7) * 0.4 * 1) + ((1287798.706 * .3) * 1 * 1)),
    #        status = round((cum_rel_prot / ref_point) * 100, 2)) %>%
    # ## v3: Reaching 60% of EEZ Protected as Cat II (KBA) = 0.8 & 40% of EEZ Protected as Category VI
    ...
    #alternative trends for reference point v2 and v3
    # ## v2: Trend status_2
    ...
    # ## v3: Trend status_3
    # lsp_trend_3 <- lsp_status_allyear %>%
    #   filter(year %in% 2012:2016) %>%
    ...
    scores_LSP <- rbind(lsp_status, lsp_trend_2) %>%
      mutate(goal = "LSP",
             rgn_id = as.integer(rgn_id)) %>%
      select(region_id = rgn_id, goal, dimension, score)
